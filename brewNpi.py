...
#GPIO Assignment
# REV1 Others                         PIN #
# 0----2-(SDA1 I2C)-Start Button---------- 3
# 1----3-(SCL1 I2C)-Reset Button---------- 5
# 4------------DS18B20 Temp Sensor-------- 7
# 5------------Buzzer     ---------------- 29
# 6------------Pump       ---------------- 31
# 7------------Next Step Button----------- 26
# 8------------Alt Switch----------------- 24
# 9----(MISO)--                  --------- 21
#10----(MOSI)--                  --------- 19
#11----(SCKL)--              ------------- 23
#12------------To Kettle Valve------------ 32
#13------------RIMS Valve ---------------- 33
#14----(TXD)---              ------------- 8
#15----(RXD)---              ------------- 10
#16------------To Mash Valve-------------- 36
#17------------LCD D5        ------------- 11
#18----(PWM)---ElementPWM  --------------- 12
#19------------To Kettle Valve------------ 35
#20------------Chiller Valve-------------- 38
#21----(PWM)---LCD D6      --------------- 40
#22------------LCD D7      --------------- 15
#23------------LCD D4      --------------- 16
...
#    sparkModule = 22
#    gasValve = 23
    waterValve = 20 #Rims Valve
    pump = 6
    KValve = 12
    MValve = 19
    ToKValve = 16
    CValve = 13
    Element = 18    # To allow power to element contactors
    ElementSel = 26 #Use a valve to control
    Contactor = 27
...
class param:
    ...
    pidKdSp = 0.0
    strikeTempSp = 130.0
    acidMashTempSp = 110.0
    acidMashTimeSp = 0
    proteinMashTempSp = 122.0
    proteinMashTimeSp = 20
    sacchMashTempSp = 149.0
    sacchMashTimeSp = 45
    sacch2MashTempSp = 154.0
    sacch2MashTimeSp = 15
    mashOutTempSp = 170.0
    mashOutTimeSp = 10.0
    hotBreakTempSp = 210.0
    hotBreakTimeSp = 1.0
    hotBreakCvSp = 25.0
    boilTimeSp = 60
    boilCvSp = 55.0
    hopStandTempSp = 190.0
    hopStandTimeSp = 0
    ...
    cmdStopBurner = False
    cmdOpenKValve = False   #From Kettle
    cmdCloseKValve = False  #From Kettle
    KValveStatus = "Closed" #From Kettle
    cmdOpenToKValve = False
    cmdCloseToKValve = False
    ToKValveStatus = "Closed"
    cmdOpenCValve = False
    cmdCloseCValve = False
    CValveStatus = "Closed"
    cmdOpenMValve = False
    cmdCloseMValve = False
    MValveStatus = "Closed"
    burnerStatus = "Off"
    KTemp = 0.0
    KTempSN = "28-0000066efc17"
    MOTempSN = "28-0000066f05b1"
    CTempSN = "28-0000066e5a8b" # Chiller Temp Probe S/N
    MTempSN = "28-000006708b31" # Mash Tank Temp Probe S/N
    RTempSN = "28-0000066ef0a8" # Mash Output Temp Probe S/N
...
class waterValve:
    def __init__(self, gpioPinNumb, strName):
        #Configure the water solenoid valve GPIO pin for output
        GPIO.setup(gpioPinNumb, GPIO.OUT, initial=GPIO.HIGH)
        self.openStatus = 'Closed'
        self.PinNumb = gpioPinNumb
        self.sName = strName

    def open(self):
        if self.sName == "Element":
           print "Turn on RIMS Element"
        else:
           print "Open " + self.sName + " Valve"

        #open the water solenoid valve
        GPIO.output(self.PinNumb, GPIO.LOW)
        self.openStatus = 'Open'
        
    def close(self):
        if self.sName == "Element":
           print "Turn on Kettle Element"
        else:
           print "Close " + self.sName + " Valve"

        #Close the water solenoid valve
        GPIO.output(self.PinNumb, GPIO.HIGH)
        self.openStatus = 'Closed'
        
    def getStatus(self):
        return self.openStatus

class PowerSW: #Switch to apply power
    def __init__(self, gpioPinNumb):
        #Configure the water solenoid valve GPIO pin for output
        GPIO.setup(gpioPinNumb, GPIO.OUT, initial=GPIO.HIGH)
...
class buzzer:
    def __init__(self):
        ...
        GPIO.setup(gpioPins.buzzer, GPIO.IN)
        
    def buzz(self):
        print "Start Buzzer"
        GPIO.setup(gpioPins.buzzer, GPIO.OUT,)
        time.sleep(2)
        print "Stop Buzzer"
        GPIO.setup(gpioPins.buzzer, GPIO.IN)
...
class ds18b20:
    def __init__(self, SerNumb):
        ...
        p = Process(target=self.readLoop, args=(self.probe, SerNumb, ))
        p.start()
    ...
    def readLoop(self, probe, SerNumb):
        base_dir = '/sys/bus/w1/devices/'
        device_folder = glob.glob(base_dir + SerNumb)[0]
...
def ProcessControl(dataLoggingStatus, pumpStatus, waterValveStatus, KValveStatus, MValveStatus, ToCValveStatus, ToKValveStatus, burnerStatus, temperature, KTemp, CTemp, MTemp, MOTemp, brewMessage, trendData, timerString, pidMode, pidPv, pidCv, pidSp, pidKpSp, \
                   ...
                   cmdStopDataLog, cmdStartPump, cmdStopPump, cmdOpenWaterValve, cmdCloseWaterValve, cmdOpenKValve, cmdCloseKValve, cmdOpenToKValve, cmdCloseToKValve, cmdOpenMValve, cmdCloseMValve, cmdOpenCValve, cmdCloseCValve, cmdStartBurner, cmdStopBurner, statusQ, conn):
        ...
        lcdStringLast = " "
        pump1 = pump()
        waterValve1 = waterValve(gpioPins.waterValve, "RIMS")
        KValve1 = waterValve(gpioPins.KValve, "Kettle Out")
        ToKValve1 = waterValve(gpioPins.ToKValve, "Kettle In")
        CValve1 = waterValve(gpioPins.CValve, "Chiller")
        MValve1 = waterValve(gpioPins.MValve, "Mash")
        ElementSel1 = waterValve(gpioPins.ElementSel, "Element") # Open for RIMS Close for Kettle
        ElementPower = PowerSW(gpioPins.Contactor)
        buzzer1 = buzzer()
        temperature1 = ds18b20(param.RTempSN)
        KTemp1 = ds18b20(param.KTempSN)
        CTemp1 = ds18b20(param.CTempSN)
        MTemp1 = ds18b20(param.MTempSN)
        MOTemp1 = ds18b20(param.MOTempSN)
        GPIO.setup(gpioPins.Element, GPIO.OUT)
        ElementPWM = GPIO.PWM(gpioPins.Element,.5)
        ElementPWM.start(0)
        pvMin = 32.0
        pvMax = 212.0
        ignitionCv = 40.0
...
#        servoController = servo()
        ...
        while (True):
            while conn.poll(): #POST settings
                dataLoggingStatus, pumpStatus, waterValveStatus, KValveStatus, ToKValveStatus, MValveStatus, CValveStatus, burnerStatus, temperature, KTemp, CTemp, MTemp, MOTemp, brewMessage, trendData, timerString, pidMode, pidPv, pidCv, pidSp, pidKpSp, \
                pidKiSp, pidKdSp, strikeTempSp, acidMashTempSp, acidMashTimeSp, proteinMashTempSp, proteinMashTimeSp, sacchMashTempSp, sacchMashTimeSp, \
                sacch2MashTempSp, sacch2MashTimeSp, mashOutTempSp, mashOutTimeSp, hotBreakTempSp, hotBreakTimeSp, hotBreakCvSp, boilTimeSp, boilCvSp, \
                hopStandTempSp, hopStandTimeSp, counterFlowTempSp, trendDurationSp, cvMinSp, cvMaxSp, pidDbSp, shutOffTempSp, cmdStart, cmdStepIncrement, cmdPidManual, cmdPidAuto, cmdReset, cmdStartDataLog, \
                cmdStopDataLog, cmdStartPump, cmdStopPump, cmdOpenWaterValve, cmdCloseWaterValve, cmdOpenKValve, cmdCloseKValve, cmdOpenToKValve, cmdCloseToKValve, cmdOpenMValve, cmdCloseMValve, cmdOpenCValve, cmdCloseCValve, cmdStartBurner, cmdStopBurner = conn.recv()

            temperature = temperature1.read()
            KTemp = KTemp1.read()
            if KTemp > 170:
              KTemp += 4.5 #seems to be an issue with my temp sensor, never goes above 207.5
            ...
            if Selector =="Kettle":
                ...
                pidKpSp = 22.27660
                pidKiSp = 1.0
                if ElementSel1.getStatus() == "Open":
                  ElementSel1.close() # Select Kettle Element

            else:
                pidPv = temperature
                pidKpSp = 12.0
                pidKiSp = 60.0
                if ElementSel1.getStatus() == "Closed":
                  ElementSel1.open()  # Select RIMS Element
            ...
            if Selector == "Kettle":
                dataLogger.logData(brewStep, timerString, KTemp, pidSp, pidCv)
            else:
            ...
            ElementPWM.ChangeDutyCycle(pidCv)
            ...
            if brewStep == "Ready":
                ...
                if cmdStart:
                    pidSp = strikeTempSp
                    pidController.cmdInitialize(cvMinSp, cvMaxSp)
                    Selector = "Kettle"
                    KValve1.open()
                    waterValve1.close()
                    MValve1.close()
            ...
            if brewStep == "Heat for Strike":
                ...
                if KTemp >= (strikeTempSp - 2):
                    pidController.cmdAuto()
                    pidController.cmdInitialize(cvMinSp, cvMaxSp)

                if KTemp >= strikeTempSp or cmdStepIncrement:
                    ElementPower.Off()
                    cmdStepIncrement = False
                    buzzer1.start()
                    brewStep = "Dough In"
            ...
            if brewStep == "Dough In":
                ...
                if cmdStepIncrement:
                  pidSp = acidMashTempSp + 2 #temperature differential between tube and mash
                  pidController.cmdInitialize(cvMinSp, cvMinSp)
                  pidController.cmdAuto()
                  ElementPower.On()
                  cmdStepIncrement = False
                  brewStep = "Heat for Acid Rest Mash"

            if brewStep == "Heat for Acid Rest Mash":
                if MTemp >= (acidMashTempSp - .5) or cmdStepIncrement:
            ...
            if brewStep == "Acid Rest Mash":
                ...
                if acidMashTimer.isDone():
                    pidSp = proteinMashTempSp + 1.5 #temperature differential between tube and mash
                    brewStep = "Heat for Protein Rest"
                    
            if brewStep == "Heat for Protein Rest":
                if MTemp >= proteinMashTempSp or cmdStepIncrement:
            ...
            if brewStep == "Protein Rest Mash":
                ...
                if proteinMashTimer.isDone():
                    pidSp = sacchMashTempSp + 1.0 #temperature differential between tube and mash
                    brewStep = "Heat for Sacch Rest"
                    
            if brewStep == "Heat for Sacch Rest":
                if MTemp >= sacchMashTempSp or cmdStepIncrement:
            ...
            if brewStep == "Sacch Rest Mash":
                ...
                if sacchMashTimer.isDone():
                    pidSp = sacch2MashTempSp + 1.0 #temperature differential between tube and mash
                    brewStep = "Heat for Sacch Rest 2"
                    
            if brewStep == "Heat for Sacch Rest 2":
                if MTemp >= sacch2MashTempSp or cmdStepIncrement:
            ...
            if brewStep == "Sacch Rest 2 Mash":
                ...
                if sacch2MashTimer.isDone():
                    pidSp = mashOutTempSp + 1.0 #temperature differential between tube and mash
                    brewStep = "Heat for Mash Out"
                    
            if brewStep == "Heat for Mash Out":

                if MTemp >= mashOutTempSp or cmdStepIncrement:
            ...
            if brewStep == "Squeeze":
                if cmdStepIncrement:
                      ...
                      ElementPower.On()
                      KValve1.open() #Stir for more even heat
                      cmdStepIncrement = False
                      brewStep = "Heat for Boil"
                    
            if brewStep == "Heat for Boil":

                if KTemp >= hotBreakTempSp or cmdStepIncrement:
                    cmdStepIncrement = False
                    buzzer1.start()
                    pidSp = 211.0
#                    pidCv = copy.copy(hotBreakCvSp)
            ...
            if brewStep == "Bring to Boil":
                if KTemp >= 207.0 or cmdStepIncrement:
                    cmdStepIncrement = False
                    buzzer1.start()
                    brewStep = "Boil"
                    pump1.stop()
           ...
           #         brewStep = "Hop Stand"
            ...
            if brewStep == "Ready":
            ...
            elif brewStep == "Heat for Acid Rest Mesh":
                brewMessage = "Heat the Mash to {0}&deg;F<br>For Acid Rest"
                ...
                brewMessage = "Moving wort to<br>the Boil Kettle"
            elif brewStep == "Heat for Boil":
                brewMessage = "Heat the Wort<br>for the Boil"
            elif brewStep == "Hot Break Control":
                brewMessage = "Reduce the Element Output<br>to Control the Hot Break"
            ...
            if brewStep == "Ready":
                ...
                lcdMessage = "    Dough In    \n    and Stir    \n"
                lcdData = ""
            elif brewStep == "Heat for Acid Rest Mash":
                lcdMessage = "  Heat Mash   \n  to {0}{1}F \n".format(acidMashTempSp,degree)
                lcdLabel = copy.copy(lcdTempLable)
                lcdData = "{0}{1}F".format(acidMashTempSp,degree)
            elif brewStep == "Acid Rest Mash":
            ...
            if lcdMsgTimer.isDone() and lcdData != "":
                if Selector =="Kettle":
                    lcdString = lcdLabel + "\n" + "{0:.1f}{1}F".format(KTemp,degree) + "    " + lcdData + "\n"
                else:
                    lcdString = lcdLabel + "\n" + "{0:.1f}{1}F".format(MTemp,degree) + "    " + lcdData + "\n"
            else:
            ...
            burnerStatus = ElementPower.getStatus()
            waterValveStatus = waterValve1.getStatus()
            KValveStatus = KValve1.getStatus()
            ToKValveStatus = ToKValve1.getStatus()
            MValveStatus = MValve1.getStatus()
            CValveStatus = CValve1.getStatus()
            
            if (not statusQ.full()):    
                statusQ.put([dataLoggingStatus, pumpStatus, waterValveStatus, KValveStatus, ToKValveStatus, MValveStatus, CValveStatus, burnerStatus, temperature, KTemp, CTemp, MTemp, MOTemp, brewMessage, trendData, timerString, pidMode, pidPv, pidCv, pidSp, pidKpSp, \
                             pidKiSp, pidKdSp, strikeTempSp, acidMashTempSp, acidMashTimeSp, proteinMashTempSp, proteinMashTimeSp, sacchMashTempSp, sacchMashTimeSp, \
                             sacch2MashTempSp, sacch2MashTimeSp, mashOutTempSp, mashOutTimeSp, hotBreakTempSp, hotBreakTimeSp, hotBreakCvSp, boilTimeSp, boilCvSp, \
                             hopStandTempSp, hopStandTimeSp, counterFlowTempSp, trendDurationSp, cvMinSp, cvMaxSp, pidDbSp, shutOffTempSp, cmdStart, cmdStepIncrement, cmdPidManual, cmdPidAuto, cmdReset, cmdStartDataLog, \
                             cmdStopDataLog, cmdStartPump, cmdStopPump, cmdOpenWaterValve, cmdCloseWaterValve, cmdOpenKValve, cmdCloseKValve, cmdOpenToKValve, cmdCloseToKValve, cmdOpenMValve, cmdCloseMValve, cmdOpenCValve, cmdCloseCValve, cmdStartBurner, cmdStopBurner]) #GET request
...
class htmlComm:
    ...
    def __init__(self):
        ...
        self.KValveStatus = param.KValveStatus
        self.ToKValveStatus = param.ToKValveStatus
        self.MValveStatus = param.MValveStatus
        self.CValveStatus = param.CValveStatus
        self.burnerStatus = param.burnerStatus
        self.temperature = param.temperature
        self.KTemp = param.KTemp
        self.CTemp = param.CTemp
        self.MTemp = param.MTemp
        self.MOTemp = param.MOTemp
        ...
        self.cmdOpenKValve = param.cmdOpenKValve
        self.cmdCloseKValve = param.cmdCloseKValve
        self.cmdOpenToKValve = param.cmdOpenToKValve
        self.cmdCloseToKValve = param.cmdCloseToKValve
        self.cmdOpenMValve = param.cmdOpenMValve
        self.cmdCloseMValve = param.cmdCloseMValve
        self.cmdOpenCValve = param.cmdOpenCValve
        self.cmdCloseCValve = param.cmdCloseCValve
    ...
    def GET(self):
        ...
        if (statusQ.full()): #remove old data
            for i in range(statusQ.qsize()):
                ...
                sacch2MashTempSp, sacch2MashTimeSp, mashOutTempSp, mashOutTimeSp, hotBreakTempSp, hotBreakTimeSp, hotBreakCvSp, boilTimeSp, boilCvSp, \
                hopStandTempSp, hopStandTimeSp, counterFlowTempSp, trendDurationSp, cvMinSp, cvMaxSp, pidDbSp, shutOffTempSp, cmdStart, cmdStepIncrement, cmdPidManual, cmdPidAuto, cmdReset, cmdStartDataLog, \
                cmdStopDataLog, cmdStartPump, cmdStopPump, cmdOpenWaterValve, cmdCloseWaterValve, cmdOpenKValve, cmdCloseKValve, cmdOpenToKValve, cmdCloseToKValve, cmdOpenMValve, cmdCloseMValve, cmdOpenCValve, cmdCloseCValve, cmdStartBurner, cmdStopBurner = web.ctx.globals.statusQ.get()
        dataLoggingStatus, pumpStatus, waterValveStatus, KValveStatus, ToKValveStatus, MValveStatus, CValveStatus, burnerStatus, temperature, KTemp, CTemp, MTemp, MOTemp, brewMessage, trendData, timerString, pidMode, pidPv, pidCv, pidSp, pidKpSp, \
        pidKiSp, pidKdSp, strikeTempSp, acidMashTempSp, acidMashTimeSp, proteinMashTempSp, proteinMashTimeSp, sacchMashTempSp, sacchMashTimeSp, \
        sacch2MashTempSp, sacch2MashTimeSp, mashOutTempSp, mashOutTimeSp, hotBreakTempSp, hotBreakTimeSp, hotBreakCvSp, boilTimeSp, boilCvSp, \
        hopStandTempSp, hopStandTimeSp, counterFlowTempSp, trendDurationSp, cvMinSp, cvMaxSp, pidDbSp, shutOffTempSp, cmdStart, cmdStepIncrement, cmdPidManual, cmdPidAuto, cmdReset, cmdStartDataLog, \
        cmdStopDataLog, cmdStartPump, cmdStopPump, cmdOpenWaterValve, cmdCloseWaterValve, cmdOpenKValve, cmdCloseKValve, cmdOpenToKValve, cmdCloseToKValve, cmdOpenMValve, cmdCloseMValve, cmdOpenCValve, cmdCloseCValve, cmdStartBurner, cmdStopBurner \
        ...
        out = json.dumps({"dataLoggingStatus" : dataLoggingStatus,
                           ...
                           "waterValveStatus" : waterValveStatus,
                             ...
                             "ToKValveStatus" : ToKValveStatus,
                               "MValveStatus" : MValveStatus,
                               "CValveStatus" : CValveStatus,
    ...
    def POST(self):
        if (statusQ.full()): #remove old data
            for i in range(statusQ.qsize()):
                self.dataLoggingStatus, self.pumpStatus, self.waterValveStatus, self.KValveStatus, self.ToKValveStatus, self.MValveStatus, self.CValveStatus, self.burnerStatus, self.temperature, self.KTemp, self.CTemp, self.MTemp, self.MOTemp, self.brewMessage, self.trendData, self.timerString, self.pidMode, self.pidPv, self.pidCv, self.pidSp, self.pidKpSp, \
                ...
                self.cmdStopDataLog, self.cmdStartPump, self.cmdStopPump, self.cmdOpenWaterValve, self.cmdCloseWaterValve, self.cmdOpenKValve, self.cmdCloseKValve, self.cmdOpenToKValve, self.cmdCloseToKValve, self.cmdOpenMValve, self.cmdCloseMValve, self.cmdOpenCValve, self.cmdCloseCValve, self.cmdStartBurner, self.cmdStopBurner = web.ctx.globals.statusQ.get()
        self.dataLoggingStatus, self.pumpStatus, self.waterValveStatus, self.KValveStatus, self.ToKValveStatus, self.MValveStatus, self.CValveStatus, self.burnerStatus, self.temperature, self.KTemp, self.CTemp, self.MTemp, self.MOTemp, self.brewMessage, self.trendData, self.timerString, self.pidMode, self.pidPv, self.pidCv, self.pidSp, self.pidKpSp, \
        self.pidKiSp, self.pidKdSp, self.strikeTempSp, self.acidMashTempSp, self.acidMashTimeSp, self.proteinMashTempSp, self.proteinMashTimeSp, self.sacchMashTempSp, self.sacchMashTimeSp, \
        self.sacch2MashTempSp, self.sacch2MashTimeSp, self.mashOutTempSp, self.mashOutTimeSp, self.hotBreakTempSp, self.hotBreakTimeSp, self.hotBreakCvSp, self.boilTimeSp, self.boilCvSp, \
        self.hopStandTempSp, self.hopStandTimeSp, self.counterFlowTempSp, self.trendDurationSp, self.cvMinSp, self.cvMaxSp, self.pidDbSp, self.shutOffTempSp, self.cmdStart, self.cmdStepIncrement, self.cmdPidManual, self.cmdPidAuto, self.cmdReset, self.cmdStartDataLog, \
        self.cmdStopDataLog, self.cmdStartPump, self.cmdStopPump, self.cmdOpenWaterValve, self.cmdCloseWaterValve, self.cmdOpenKValve, self.cmdCloseKValve, self.cmdOpenToKValve, self.cmdCloseToKValve, self.cmdOpenMValve, self.cmdCloseMValve, self.cmdOpenCValve, self.cmdCloseCValve, self.cmdStartBurner, self.cmdStopBurner \
        = web.ctx.globals.statusQ.get()
        ...
        for key in webData:
            ...
            if key == "cmdOpenKValve":
                print webData[key]
                self.cmdOpenKValve = webData[key]
            if key == "cmdCloseKValve":
                print webData[key]
                self.cmdCloseKValve = webData[key]
            if key == "cmdOpenToKValve":
                print webData[key]
                self.cmdOpenToKValve = webData[key]
            if key == "cmdCloseToKValve":
                print webData[key]
                self.cmdCloseToKValve = webData[key]
            if key == "cmdOpenMValve":
                print webData[key]
                self.cmdOpenMValve = webData[key]
            if key == "cmdCloseMValve":
                print webData[key]
                self.cmdCloseMValve = webData[key]
            if key == "cmdOpenCValve":
                print webData[key]
                self.cmdOpenCValve = webData[key]
            if key == "cmdCloseCValve":
                print webData[key]
                self.cmdCloseCValve = webData[key]
        ...
        web.ctx.globals.parent_conn.send([self.dataLoggingStatus, self.pumpStatus, self.waterValveStatus, self.KValveStatus, self.ToKValveStatus, self.MValveStatus, self.CValveStatus, self.burnerStatus, self.temperature, self.KTemp, self.CTemp, self.MTemp, self.MOTemp, self.brewMessage, self.trendData, self.timerString, self.pidMode, self.pidPv, self.pidCv, self.pidSp, self.pidKpSp, \
                                          self.pidKiSp, self.pidKdSp, self.strikeTempSp, self.acidMashTempSp, self.acidMashTimeSp, self.proteinMashTempSp, self.proteinMashTimeSp, self.sacchMashTempSp, self.sacchMashTimeSp, \
                                          self.sacch2MashTempSp, self.sacch2MashTimeSp, self.mashOutTempSp, self.mashOutTimeSp, self.hotBreakTempSp, self.hotBreakTimeSp, self.hotBreakCvSp, self.boilTimeSp, self.boilCvSp, \
                                          self.hopStandTempSp, self.hopStandTimeSp, self.counterFlowTempSp, self.trendDurationSp, self.cvMinSp, self.cvMaxSp, self.pidDbSp, self.shutOffTempSp, self.cmdStart, self.cmdStepIncrement, self.cmdPidManual, self.cmdPidAuto, self.cmdReset, self.cmdStartDataLog, \
                                          self.cmdStopDataLog, self.cmdStartPump, self.cmdStopPump, self.cmdOpenWaterValve, self.cmdCloseWaterValve, self.cmdOpenKValve, self.cmdCloseKValve, self.cmdOpenToKValve, self.cmdCloseToKValve, self.cmdOpenMValve, self.cmdCloseMValve, self.cmdOpenCValve, self.cmdCloseCValve, self.cmdStartBurner, self.cmdStopBurner])
    ...
    def GET(self):
        ...
        if (statusQ.full()): #remove old data
            for i in range(statusQ.qsize()):
                ...
                sacch2MashTempSp, sacch2MashTimeSp, mashOutTempSp, mashOutTimeSp, hotBreakTempSp, hotBreakTimeSp, hotBreakCvSp, boilTimeSp, boilCvSp, \
                hopStandTempSp, hopStandTimeSp, counterFlowTempSp, trendDurationSp, cvMinSp, cvMaxSp, pidDbSp, shutOffTempSp, cmdStart, cmdStepIncrement, cmdPidManual, cmdPidAuto, cmdReset, cmdStartDataLog, \
                cmdStopDataLog, cmdStartPump, cmdStopPump, cmdOpenWaterValve, cmdCloseWaterValve, cmdCloseKValve, cmdOpenKValve, cmdCloseToKValve, cmdOpenToKValve, cmdCloseMValve, cmdOpenMValve, cmdCloseCValve, cmdOpenCValve, cmdStartBurner, cmdStopBurner = web.ctx.globals.statusQ.get()
        dataLoggingStatus, pumpStatus, waterValveStatus, KValveStatus, ToKValveStatus, MValveStatus, CValveStatus, burnerStatus, temperature, KTemp, CTemp, MTemp, MOTemp, brewMessage, trendData, timerString, pidMode, pidPv, pidCv, pidSp, pidKpSp, \
        pidKiSp, pidKdSp, strikeTempSp, acidMashTempSp, acidMashTimeSp, proteinMashTempSp, proteinMashTimeSp, sacchMashTempSp, sacchMashTimeSp, \
        sacch2MashTempSp, sacch2MashTimeSp, mashOutTempSp, mashOutTimeSp, hotBreakTempSp, hotBreakTimeSp, hotBreakCvSp, boilTimeSp, boilCvSp, \
        hopStandTempSp, hopStandTimeSp, counterFlowTempSp, trendDurationSp, cvMinSp, cvMaxSp, pidDbSp, shutOffTempSp, cmdStart, cmdStepIncrement, cmdPidManual, cmdPidAuto, cmdReset, cmdStartDataLog, \
...
if __name__ == '__main__':
    ...
    render = web.template.render('/home/pi/BrewNpi/templates/')

    app = web.application(urls, globals()) 
    
    statusQ = Queue(2)       
    parent_conn, child_conn = Pipe()
    p = Process(name = "ProcessControl", target=ProcessControl, args=(param.dataLoggingStatus, param.pumpStatus, param.waterValveStatus, param.KValveStatus, param.ToKValveStatus, param.MValveStatus, param.CValveStatus, param.burnerStatus, param.temperature, param.KTemp, param.CTemp, param.MTemp, param.MOTemp, param.brewMessage, param.trendData, param.timerString, param.pidMode, param.pidPv, param.pidCv, param.pidSp, param.pidKpSp, \
                                                                      ...
                                                                      param.cmdStopDataLog, param.cmdStartPump, param.cmdStopPump, param.cmdOpenWaterValve, param.cmdCloseWaterValve, param.cmdOpenKValve, param.cmdCloseKValve, param.cmdOpenToKValve, param.cmdCloseToKValve, param.cmdOpenMValve, param.cmdCloseMValve, param.cmdOpenCValve, param.cmdCloseCValve, param.cmdStartBurner, param.cmdStopBurner,\
                                                                      statusQ, child_conn))
    p.start()
